...
```json
{
  ...
  "properties": {
    "content": {
      "type": "string",
      "description": "ホワイトボードに表示する文章。7行程度を目安にし、URLやリンク付きテキストは含めないでください。"
    }
  },
  "required": ["content"],
...
- `wschat` 側では `whiteboard_update` を生成していますが、その先のフロントエンドがどのコンポーネントでどう描画しているかは、今回の参照範囲では扱っていません。

## 不明
- ホワイトボード表示の最終的な UI レイアウト、改行表示ルール、文字数上限は、この参照範囲からは不明です。
- `whiteboard_update` を受けたフロントエンド側が差分更新か全置換かのどちらで描画しているかは、この参照範囲からは不明です。

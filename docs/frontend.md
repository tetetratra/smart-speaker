...
- **対象範囲**: `web/` 配下の Web UI、通常画面、ログ管理画面、メモリ画面、接続開始、WebSocket / WebRTC の接続ライフサイクル、ブラウザ側の主な責務。
...
### 2.2 表示モード
- 現行フロントは 1 つの React アプリで通常画面、ログ管理画面、メモリ画面を切り替える構成である。
- 初期モードは URL から決まる。
...
- 保存済みメモリの件数、本文、タグ、作成日時、更新日時を表示する。
- ユーザー入力のメモリ文を追加できる。
...
### 4.4 URL と表示モード
- URL から表示モードを決定する。
- `pushState` / `popstate` で通常画面、ログ管理画面、メモリ画面を切り替える。
- ログ管理画面またはメモリ画面の表示中は `body.admin-mode` を付与する。
...
- 通常画面、ログ管理画面、メモリ画面は別アプリではなく、同一 state の別表示である。

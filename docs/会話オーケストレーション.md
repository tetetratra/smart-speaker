...
- **`response_contract.go`**
  ...
  - `parseAIOutput` が `timeline + optional whiteboard` の契約を検証する。
...
2. **契約検証**: `parseAIOutput` が `timeline + optional whiteboard` の JSON 契約を検証する。
...
6. **白板更新**: `whiteboard` が含まれる場合だけ `EventWhiteboardUpdate` を emit する。
  ...
  - `components/`
    - `conversation/`
      ...
      - `rule_responses.go`: LLM 応答を扱う rule。
        - `Apply`: 応答契約の検証、whiteboard emit、timeline 反映を行う。
      ...
      - `response_contract.go`: LLM 応答 JSON の DTO と契約検証。
        - `parseAIOutput`: `timeline + optional whiteboard` の妥当性を検証する。

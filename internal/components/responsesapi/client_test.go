...
func TestCreateResponseStream(t *testing.T) {
	...
	t.Run("completed responseからtool callを抽出する", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header().Set("Content-Type", "text/event-stream")
			_, _ = w.Write([]byte("data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_tool\",\"output\":[{\"type\":\"function_call\",\"name\":\"set_volume\",\"call_id\":\"call_1\",\"arguments\":\"{\\\"level\\\":5}\"}]}}\n\n"))
		}))
		defer server.Close()
		...
		resp, err := client.CreateResponseStream(
			context.Background(),
			[]types.ChatMessage{{Role: "user", Content: "音量上げて"}},
			"",
			nil,
			nil,
			nil,
		...
		if call.ResponseID != "resp_tool" || call.ToolCallID != "call_1" || call.Name != "set_volume" {
